...
$(document).ready(function(){
	strechfondo();
	centerModal();
	$('#pestana1 a').click(function (e) {
	  e.preventDefault()
	  $(this).tab('show')
	});
	$('#pestana2 a').click(function (e) {
	  e.preventDefault()
	  $(this).tab('show')
	});
	$('#pestana3 a').click(function (e) {
	  e.preventDefault()
	  $(this).tab('show')
	});
...
function AgregarBase(cual){
	var existesalsa=$('#tablabase .agregado');
	...
	col.innerHTML="<div class='agregado' tipo='1' id='agregado_"+cual.getAttribute('id')+"'><div class='label label-danger' style='margin:3px; cursor:pointer;' onclick='QuitarIngrediente("+cual.getAttribute('id')+");'>X</div><span class='nombreagregado'>"+cual.getAttribute('nombre')+"</span></div>";
	document.getElementById('tablabase').appendChild(fila);
	Pasar();
}

function AgregarIngrediente(cual){
	var existesalsa=$('#tablaagregados .agregado');
	if(existesalsa.length>0){
		var data=existesalsa.attr('id').split('_');
		totalcal-=parseFloat($('#'+data[1]).attr('cal'));
		totalgram-=parseFloat($('#'+data[1]).attr('gram'));
	...
	var col=document.createElement('td');
	fila.appendChild(col);
	col.innerHTML="<div class='agregado' tipo='2' id='agregado_"+cual.getAttribute('id')+"'><div class='label label-danger' style='margin:3px; cursor:pointer;' onclick='QuitarIngrediente("+cual.getAttribute('id')+");'>X</div><span class='nombreagregado'>"+cual.getAttribute('nombre')+"</span></div>";
...
function AgregarSalsa(cual){
	...
	var col=document.createElement('td');
	fila.appendChild(col);
	col.innerHTML="<div class='agregado' tipo='3' id='agregado_"+cual.getAttribute('id')+"'><div class='label label-danger' style='margin:3px; cursor:pointer;' onclick='QuitarIngrediente("+cual.getAttribute('id')+");'>X</div><span class='nombreagregado'>"+cual.getAttribute('nombre')+"</span></div>";
...
function QuitarIngrediente(elid){
	var elemento=$('#'+elid);
	var tipo=$('#agregado_'+elid).attr('tipo');
	$('#pestana'+tipo+' a').click();
...
function Pasar(){
	var c=0;
	$('#componentes .agregado').each(function(){
		c++;
	});
	paso=c;
	if(paso==1){
	...
	var sal=$('#tablasalsa .agregado');
	if(bas.length==0)
		$('#pestana1 a').click();
	else if(agre.length==0)
		$('#pestana2 a').click();
...
function VerCuenta(){
	...
	for(var n=0;n<platos.length;n++){
		...
		fila.innerHTML="<td><div>Plato"+(n+1)+": </div></td><td>"+platos[n][6]+"</td><td>"+platos[n][5].toFixed(2)+"</td><td>"+ptotal.toFixed(2)+"</td><td><button type='button' id='btnpop"+n+"' class='btn btn-default' data-container='body' data-toggle='popover' data-placement='right' data-content='"+"<b>Huevos: </b>"+mibase+"<br/><b>Pan: </b>"+cadfavoritos+"<br/><b>Café/Jugo: </b>"+misalsa+"'>Detalles</button></td>";
